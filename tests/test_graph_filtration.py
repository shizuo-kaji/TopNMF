...
def test_graph_filtration_rejects_negative_max_dim(graph_filtration_module):
    with pytest.raises(ValueError, match="max_dim must be non-negative"):
        graph_filtration_module.GraphFiltrationPH(max_dim=-1)


def test_canonical_edge_and_lookup_treat_graph_as_undirected(graph_filtration_module):
    graph_ph = graph_filtration_module.GraphFiltrationPH

    assert graph_ph._canonical_edge((5, 2)) == (2, 5)

    edge_lookup = graph_ph._build_edge_lookup([(2, 0), (0, 2), (1, 3)])
    assert edge_lookup[(0, 2)] == 0
    assert edge_lookup[(1, 3)] == 2


def test_prepare_inputs_rejects_empty_edges_and_mismatched_weights(
    graph_filtration_module,
    torch,
):
    model = graph_filtration_module.GraphFiltrationPH()
...
def test_prepare_inputs_superlevel_flips_sign_and_enables_grad(
    graph_filtration_module,
    torch,
):
    all_edges = [(0, 1), (2, 1)]
    ...
    sublevel_model = graph_filtration_module.GraphFiltrationPH(superlevel=False)
    ...
    superlevel_model = graph_filtration_module.GraphFiltrationPH(superlevel=True)
...
def test_max_filtration_value_handles_empty_and_nonempty_simplex_tree(
    graph_filtration_module,
    torch,
):
    graph_ph = graph_filtration_module.GraphFiltrationPH

    empty_tree = graph_filtration_module.gudhi.SimplexTree()
    empty_max = graph_ph._max_filtration_value(empty_tree, device=torch.device("cpu"))
    assert float(empty_max) == pytest.approx(0.0)

    simplex_tree = graph_filtration_module.gudhi.SimplexTree()
    simplex_tree.insert([0], filtration=-1.0)
    simplex_tree.insert([1], filtration=-1.0)
    simplex_tree.insert([0, 1], filtration=0.4)
    nonempty_max = graph_ph._max_filtration_value(simplex_tree, device=torch.device("cpu"))
    assert float(nonempty_max) == pytest.approx(0.4)


def test_birth_value_raises_for_unknown_creator_edge(graph_filtration_module, torch):
    model = graph_filtration_module.GraphFiltrationPH()
    simplex_tree = graph_filtration_module.gudhi.SimplexTree()
...
def test_death_value_falls_back_to_max_for_unknown_edge(graph_filtration_module, torch):
    model = graph_filtration_module.GraphFiltrationPH()
    edge_values = torch.tensor([0.3], dtype=torch.float32, requires_grad=True)
    vertex_values = torch.full((2,), -1.0, dtype=torch.float32, requires_grad=True)
...
def test_forward_path_graph_returns_expected_h0_and_empty_h1(graph_filtration_module, torch):
    model = graph_filtration_module.GraphFiltrationPH(max_dim=1, superlevel=False)
...
def test_forward_superlevel_negates_edge_filtration(graph_filtration_module, torch):
    model = graph_filtration_module.GraphFiltrationPH(max_dim=0, superlevel=True)
    persistence_info = model(
        all_edges=[(0, 1), (1, 2)],
        edge_weight=[0.2, 0.8],
    )
